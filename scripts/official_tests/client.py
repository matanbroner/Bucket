...
class Client:
    ...
    def putKey(self, key, value, port):
        result = requests.put(
            "http://%s:%s/kvs/keys/%s" % (localhost, str(port), key),
            timeout=timeout,
    ...
    def getKey(self, key, port):
        ...
        print("GET RESULT {}".format(result.content))
    ...
    def viewChange(self, view, repl_factor, port):
        result = requests.put(
            "http://%s:%s/kvs/view-change" % (localhost, str(port)),
            timeout=timeout,
    ...
    def keyCount(self, port):
        result = requests.get(
            "http://%s:%s/kvs/key-count" % (localhost, str(port)),
            timeout=timeout,
            json={},
            headers={"Content-Type": "application/json"},
        )
    ...
    def getShards(self, port):
        result = requests.get(
            "http://%s:%s/kvs/shards" % (localhost, str(port)),
            timeout=timeout,
            json={},
            headers={"Content-Type": "application/json"},
        )
    ...
    def getShard(self, port, shard_id):
        result = requests.get(
            "http://%s:%s/kvs/shards/%s" % (localhost, str(port), str(shard_id)),
            timeout=timeout,
            json={},
            headers={"Content-Type": "application/json"},
        )
